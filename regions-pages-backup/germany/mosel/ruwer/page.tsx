...
// VDP Vineyard Classifications for Ruwer
const VINEYARDS = {
  grosseLage: [
...
export default async function RuwerPage() {
  // Read and parse markdown file
  const guidesDir = path.join(process.cwd(), 'guides');
  const filePath = path.join(guidesDir, 'ruwer-guide.md');
      ...
      <div className="flex gap-8">
            ...
            <section className="mb-6">
              ...
              <nav className="space-y-1">
                {VINEYARDS.grosseLage.map((vineyard) => (
                  <Link
                    key={vineyard.slug}
                    href={`/regions/germany/mosel/ruwer/${vineyard.slug}`}
        ...
        <main className="flex-1 max-w-5xl">
          ...
          <header className="mb-8 pb-6 border-b border-gray-200">
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Ruwer</h1>
            <p className="text-gray-600 text-lg">Ruwer Valley, Mosel, Germany</p>
          </header>
