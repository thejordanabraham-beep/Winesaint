...
export default async function NorthernRhonePage() {
  ...
  const filePath = path.join(guidesDir, 'northern-rhône-guide.md');
        ...
        <aside className="w-64 flex-shrink-0 hidden lg:block">
          <div className="sticky top-8 max-h-[calc(100vh-4rem)] overflow-y-auto pr-2">
            ...
            <nav className="space-y-1">
              {NORTHERN_RHONE_VILLAGES.map((village) => (
                <Link
                  key={village.slug}
                  href={`/regions/france/northern-rhone/${village.slug}`}
