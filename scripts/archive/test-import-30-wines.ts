/**
 * TEST IMPORT: 30 Random Wines from Excel
 *
 * - 10 random Austria wines
 * - 10 random California wines
 * - 10 random Italy wines
 *
 * Features:
 * - Randomized score conversion (100-point → 1-10 scale)
 * - François RAG integration for review rewording
 * - WineSaint academic voice
 * - Extracts drinking window, flavor profile
 */
...
function convertScoreRandomized(score100: number): number {
  ...
  if (range) {
    const [min, max] = range;
    // Randomize within range, 1 decimal place
  ...
  return 6.0; // Minimum for scores below 87
}
...
async function importWineWithReview(wine: any, rowNumber: number): Promise<boolean> {
  console.log(`\n[${rowNumber}] 🍷 ${wine.Producer} - ${wine['Wine Name']} (${wine.Vintage})`);
...
// Main function
async function main() {
  console.log('\n🍷 TEST IMPORT: 30 RANDOM WINES');
  console.log('='.repeat(70));
  console.log('Importing from: WINESAINT_MSTR_RVW.xlsx');
  console.log('Strategy: 10 random from each sheet (Austria, California, Italy)');
  console.log('='.repeat(70));

  // Read Excel file
  const workbook = XLSX.readFile(EXCEL_FILE);

  const results: { sheet: string; success: number; failed: number }[] = [];

  for (const sheetName of ['Austria', 'California', 'Italy']) {
    console.log(`\n\n📋 SHEET: ${sheetName}`);
    console.log('='.repeat(70));
    ...
    // Select 10 random wines
    const shuffled = allRows.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, 10);

    console.log(`Selected 10 random wines for import\n`);
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      const imported = await importWineWithReview(wine, i + 1);
    ...
    results.push({ sheet: sheetName, success, failed });
  }
