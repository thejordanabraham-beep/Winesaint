/**
 * RESTRUCTURE BARBARESCO DIRECTORY
 *
 * Transforms flat MGA structure into hierarchical commune-based structure:
 * barbaresco/[65 MGAs] → barbaresco/[4 communes]/[MGAs per commune]
 *
 * Steps:
 ...
 * 5. Update barbaresco/page.tsx to show communes
 */

import fs from 'fs';
import path from 'path';

const BARBARESCO_DIR = path.join(process.cwd(), 'app', 'regions', 'italy', 'piedmont', 'barbaresco');
const MAPPING_FILE = path.join(process.cwd(), 'data', 'barbaresco-commune-mapping.json');
...
const COMMUNE_DATA: Record<string, { name: string; slug: string }> = {
  'barbaresco': { name: 'Barbaresco', slug: 'barbaresco' },
  'neive': { name: 'Neive', slug: 'neive' },
  'treiso': { name: 'Treiso', slug: 'treiso' },
  'san-rocco-seno-d-elvio': { name: "San Rocco Seno d'Elvio", slug: 'san-rocco-seno-d-elvio' },
};

async function main() {
  console.log('\n🍷 RESTRUCTURING BARBARESCO DIRECTORY');
  ...
  for (const communeSlug of mgasByCommune.keys()) {
    const communeDir = path.join(BARBARESCO_DIR, communeSlug);
  ...
  for (const mga of mapping) {
    const mgaDir = path.join(BARBARESCO_DIR, mga.mgaSlug);
    const targetDir = path.join(BARBARESCO_DIR, mga.commune, mga.mgaSlug);
  ...
  for (const mga of mapping) {
    const mgaPageFile = path.join(BARBARESCO_DIR, mga.commune, mga.mgaSlug, 'page.tsx');
    ...
    try {
      ...
      const oldParentRegion = 'italy/piedmont/barbaresco';
      const newParentRegion = `italy/piedmont/barbaresco/${mga.commune}`;
  ...
  for (const [communeSlug, mgas] of mgasByCommune.entries()) {
    const communePageFile = path.join(BARBARESCO_DIR, communeSlug, 'page.tsx');
...
`;
  ...
  // Step 5: Update main barbaresco/page.tsx
  console.log('\n📝 Step 5: Updating barbaresco/page.tsx...');

  const barbarescoPageFile = path.join(BARBARESCO_DIR, 'page.tsx');
  ...
  const barbarescoPageContent = `import RegionLayout from '@/components/RegionLayout';

const BARBARESCO_COMMUNES = ${JSON.stringify(communes, null, 2)} as const;
...
`;
  ...
  fs.writeFileSync(barbarescoPageFile, barbarescoPageContent);
  console.log(`   ✓ Updated barbaresco/page.tsx (now shows ${communes.length} communes)`);
  ...
  console.log(`\nNew structure:`);
  console.log(`  barbaresco/ (shows ${communes.length} communes)`);
  for (const [commune, mgas] of mgasByCommune.entries()) {
