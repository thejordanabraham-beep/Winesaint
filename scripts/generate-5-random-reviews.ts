...
async function main() {
  console.log('🎲 GENERATING 5 RANDOM WINE REVIEWS');
  console.log('='.repeat(70));

  // Get wines with regions (excludes spirits) without existing reviews
  const wines = await client.fetch(`*[_type == 'wine' && defined(region) && !defined(*[_type == 'review' && wine._ref == ^._id][0]._id)] | order(_createdAt desc)[0...200] {
    _id,
    name,
    vintage,
    'slug': slug.current,
  ...
  // Pick 5 random from the first 100
  const selectedWines = [];
  const usedIndices = new Set();
  while (selectedWines.length < 5 && usedIndices.size < wines.length) {
    const randomIndex = Math.floor(Math.random() * Math.min(100, wines.length));
  ...
  console.log(`\nSelected ${selectedWines.length} wines for review:\n`);
