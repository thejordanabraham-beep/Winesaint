...
**Level:** Wine Program Management Mastery — Events & Special Programming
**Module:** 17
...
**Prerequisites:** Wine Saint Certified Level 2 or equivalent; Modules 1–16 recommended
...
## Section 1: Why Events Are a High-Margin Revenue Channel
...
## Section 2: Winemaker Dinners, Planning, Pricing, and Execution
...
| Line Item | Per Person | Total (40 guests) |
|---|---|---|
| Ticket revenue | $195 | $7,800 |
| Food cost (4-course, 28%) | $54.60 | $2,184 |
| Wine cost (6 wines, ~5 oz each) | $42.00 | $1,680 |
| Collateral (menus, florals, décor) | $8.00 | $320 |
| Event-specific labor | $18.50 | $740 |
| Total variable cost | $123.10 | $4,924 |
| **Contribution margin** | **$71.90** | **$2,876** |
| **Contribution margin %** | **36.9%** | |
...
## Section 3: Wine Tasting Events, Formats, Ticket Pricing, and Guest Experience
...
| Line Item | Per Person | Total (60 guests) |
|---|---|---|
| Ticket revenue | $85 | $5,100 |
| Wine cost (6 pours × ~2 oz, cost-per-pour ~$2.80) | $16.80 | $1,008 |
| Food cost (charcuterie, cheese: 14% of ticket) | $11.90 | $714 |
| Labor (3 staff × 3 hours × $28 loaded) | $4.20 | $252 |
| Supplies, printing, rentals | $3.00 | $180 |
| Total variable cost | $35.90 | $2,154 |
| **Contribution margin** | **$49.10** | **$2,946** |
| **Contribution margin %** | **57.8%** | |
...
## Section 4: Private Dining and Corporate Events, Custom Wine Programs and Pricing
...
**Deposit and contract terms.** Private events should always be contracted with a deposit. Industry standard is 25–50% of the estimated total event cost at time of booking, with the balance due within 72 hours of event completion. The contract should specify: minimum spend, cancellation terms (graduated, with the deposit non-refundable beyond 30 days prior), corkage policy if the client wishes to bring special bottles, and any room fee if applicable. Operators who run private events without contracts expose themselves to last-minute cancellations, scope creep, and clients who dispute charges after the event.

**Corporate accounts as recurring revenue.** A corporate client who runs six events per year at an average of $4,200 in wine revenue per event is generating $25,200 in annual wine revenue at standard event margins, roughly $17,000 in annual gross margin from a single client relationship. This is the reason corporate event relationships deserve dedicated management attention. Assign one person as the primary contact for each major corporate account. Review the prior event's performance before each new booking. Propose seasonal program updates, vintage changes, or theme refinements proactively. The corporate event client who feels actively curated does not shop competitors.
...
## Section 5: Partnerships, Working with Importers, Distributors, and Winery Reps
...
## Section 6: Measuring Event ROI, Contribution Margin, Guest Acquisition, and Long-Term Value
...
| Term | Definition |
|------|------------|
| Contribution Margin | Revenue minus all variable costs directly attributable to an event or service period. The primary measure of what an event or program contributes to fixed cost coverage and profit before overhead allocation. |
| Opportunity Cost | The foregone contribution of the alternative use of a resource, typically, the regular-service revenue and margin a restaurant gives up by reserving a room or dining area for an event. |
| Breakeven Analysis | The calculation of the minimum ticket price or cover count at which an event's revenue exactly covers its direct costs, producing zero contribution margin. Events priced below breakeven destroy value. |
| Co-op Contribution | A per-head payment made by a winery, importer, or distributor to offset event costs in exchange for product placement, promotion, and winemaker presence. Standard range: $10–$25 per guest. |
| Non-Refundable Deposit | A partial payment collected at registration that is retained by the operator in the event of guest cancellation. The primary tool for managing no-show risk in ticketed events. |
| Event P&L | A per-event profit and loss statement capturing ticket revenue, food cost, wine cost, event-specific labor, collateral costs, and contribution margin. Should be built before the event is approved and reconciled after it closes. |
| Guest Acquisition Cost | The marketing and non-recoverable direct costs of an event divided by the number of first-time guests acquired. Used to evaluate events as a guest acquisition channel against alternatives such as digital advertising or PR. |
| Long-Term Guest Value | The estimated cumulative revenue or margin generated by a guest relationship over a defined period, typically 12 months. Event guests typically demonstrate 1.5–2.5x the long-term value of average new guests due to higher return frequency and elevated wine spend. |
| Package Pricing | A fixed per-person or per-event beverage price that bundles a defined wine selection and service level into a single charge. Simplifies budgeting for corporate clients and provides predictable margin structure for the operator. |
| Hybrid Package | A private event beverage structure combining a base per-person package with optional à la carte upgrades. Generates higher average revenue per guest than fixed packages while maintaining a committed floor price for the client. |
| Incremental Contribution | The additional contribution margin an event generates above what regular service in the same space and time period would have produced. The definitive measure of whether an event is financially worthwhile to run. |
| Sell-Through Rate | The percentage of available event tickets sold prior to the event. Consistently low sell-through rates (below 70–75% without discounting) indicate a pricing, marketing, or format problem requiring diagnosis. |
| Post-Event Purchase Commitment | An informal or contractual agreement with a supplier to purchase product following a supplier-funded event. Standard practice when a winery provides free product allocation; terms should be documented to avoid disputes. |
| Rep Dependency | A program condition in which event programming and regular list decisions are driven primarily by supplier funding availability rather than guest value and product quality. A structural risk to program credibility and guest trust. |
| Event Tracking Ledger | An ongoing operational record of event financial performance (contribution margin, guest counts, new guest acquisition, and post-event revenue) used to evaluate format performance and inform future event programming decisions. |
...
**Q1. A winemaker dinner generates $2,876 in contribution margin. Regular service in the same space on the same evening would have produced $1,850 in contribution margin. What is the incremental contribution of the event?**
- A) $2,876
- B) $1,850
- C) $1,026
- D) $4,726
...
**Q2. You are pricing a 40-person winemaker dinner. Variable cost per person is $123. Your target contribution margin is 38%. What is the minimum ticket price?**
- A) $155
- B) $175
- C) $198
- D) $215
...
**Q3. Which of the following best explains why events can outperform regular service on a per-cover contribution margin basis?**
- A) Events require less skilled labor, reducing the overall cost structure
- B) Revenue is largely pre-committed before the event begins, eliminating the variability of regular service
- C) Event food costs are always lower than regular dining menu costs
- D) Events allow operators to use lower-quality wines at higher prices without guest objection

**Correct: B**

---

**Q4. A winery rep offers to fund a winemaker dinner with $20 per head in co-op support and six free bottles of wine, in exchange for adding their Sauvignon Blanc to your BTG list. The wine is technically sound but significantly underpriced relative to your list architecture. What is the most appropriate response?**
- A) Accept the terms; co-op funding improves event margins and the BTG placement is a fair trade
- B) Accept the co-op funding but negotiate the BTG placement out of the agreement; list decisions should remain based on product merit
- C) Decline the offer entirely; any rep funding creates a conflict of interest
- D) Accept the terms and place the wine on the BTG list at a higher price to protect list architecture

**Correct: B**

---

**Q5. A ticketed tasting event sells 60 tickets at $85 each. Wine cost is $16.80 per person, food cost is $11.90, labor is $4.20, and supplies are $3.00. What is the total contribution margin for the event?**
- A) $2,340
- B) $2,706
- C) $2,946
- D) $3,180
...
**Q6. Which private event beverage pricing structure typically generates the highest average revenue per guest?**
- A) Fixed package pricing at a single tier
- B) Per-bottle pricing from a private event list only
- C) Hybrid package with base inclusion and à la carte upgrades
- D) Consumption billing against the regular wine list at standard prices

**Correct: C**

---

**Q7. At a 60-person tasting event, 15 guests purchase bottles to take home at an average retail price of $48. If wine gross margin is 68%, what is the incremental contribution from post-event bottle sales?**
- A) $310
- B) $420
- C) $490
- D) $720
...
**Q8. The most common cause of margin destruction in ticketed events is:**
- A) Over-purchasing wine that cannot be returned to the distributor
- B) Insufficient marketing spend leading to under-attendance
- C) Guest no-shows on seats where food and wine cost has already been incurred
- D) Under-qualified staff requiring additional service labor

**Correct: C**

---

**Q9. A non-refundable deposit policy at ticket purchase primarily serves to:**
- A) Reduce the administrative burden of managing guest payment timelines
- B) Signal premium positioning that attracts higher-spending guests
- C) Reduce no-show rates and at minimum recover direct costs on seats that go empty
- D) Comply with credit card processing requirements for ticketed events

**Correct: C**

---

**Q10. You spend $400 on marketing and event-specific setup for a 50-person tasting. Eighteen attendees are first-time guests. What is your guest acquisition cost per new guest?**
- A) $8.00
- B) $14.00
- C) $22.22
- D) $28.57
...
**Q11. A corporate client runs six events per year at an average of $4,200 in wine revenue per event at a 68% gross margin. What is this client's annual wine gross margin contribution?**
- A) $13,440
- B) $17,136
- C) $25,200
- D) $19,800
...
**Q12. Which of the following is the strongest signal that a winemaker dinner series should be repriced upward?**
- A) Guests consistently post positive reviews on social media following the events
- B) The events consistently sell out at the current price without discounting
- C) The winery rep is requesting a higher co-op contribution for future events
- D) The contribution margin is above the regular-service average for the space

**Correct: B**

---

**Q13. A beverage director structures rep partnerships by negotiating event funding independently from regular list placement decisions. The primary reason for this approach is:**
- A) It is required under state alcohol beverage control regulations
- B) It prevents suppliers from tracking their program performance against competitors
- C) It preserves the program's credibility with guests, whose trust in recommendations is a direct driver of wine attachment and premium spend
- D) It produces higher co-op contributions because suppliers respect operators who negotiate hard

**Correct: C**

---

**Q14. Which event format typically produces the highest contribution margin percentage when well-executed?**
- A) Four-course winemaker dinner with full kitchen production
- B) Station tasting with charcuterie and cheese service
- C) Seated comparative tasting with light snacks and water
- D) Private corporate dinner with a three-tier wine package

**Correct: C**

---

**Q15. You are building an annual events program. After 12 months of tracking, your winemaker dinner series generates an average incremental contribution of $950 per event, while your seated tasting series generates $2,650 per event. All else being equal, what should this data drive in your programming decisions?**
- A) Eliminate the winemaker dinner series as it is underperforming
- B) Increase the ticket price of winemaker dinners to match the tasting series margins
- C) Shift calendar weighting toward the tasting format while maintaining the winemaker series for positioning and relationship value at a reduced frequency
- D) Reduce the quality of food production on winemaker dinners to improve their contribution margin

**Correct: C**

---

**Q16. A winery provides six bottles of free wine for a 40-person dinner at a producer price of $85 per bottle. What is the total product cost relief to your event P&L?**
- A) $340
- B) $425
- C) $510
- D) $680
...
**Q17. What is the recommended planning horizon for a high-profile winemaker dinner involving international winery travel?**
- A) Two to three weeks
- B) Four to six weeks
- C) Six to eight weeks
- D) Twelve or more weeks

**Correct: D**

---

**Q18. An operator prices a winemaker dinner at $145 per person. Variable cost per person is $123. Total contribution for a 40-person dinner is $880. Regular service in the same space that evening would have produced $1,850 in contribution margin. What does this analysis show?**
- A) The event is profitable and represents sound event programming
- B) The event is priced below the level required to generate positive incremental contribution; the operator would have been better served by running regular service
- C) The event should be repriced upward for the next iteration, but current pricing is acceptable for a first-time format
- D) The analysis cannot be completed without knowing the winery's co-op contribution

**Correct: B**

---

**Q19. Which post-event action is most effective at securing preferred placement in a supplier's future winemaker event and winery visit calendar?**
- A) Placing a large post-event purchase order within 72 hours of the dinner
- B) Sending a one-page event summary, attendance, revenue, wines poured, guest feedback, within 48 hours of the event
- C) Publicly tagging the winery on social media with event photographs
- D) Inviting the winery rep to attend the next staff training session

**Correct: B**

---

**Q20. Your average annual guest value is $380. Event guests return at 2.3x the frequency of average new guests and spend 20% above the regular check average. Assuming visit frequency is the primary driver and the spend premium applies across all visits, what is the approximate annual value of a guest acquired through an event?**
- A) $456
- B) $617
- C) $742
- D) $874
