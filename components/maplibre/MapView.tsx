...
import { COUNTRY_COLORS, REGION_COLORS, SUBREGION_COLORS } from '@/lib/maplibre/colorPalette';
import { getDepth } from '@/lib/maplibre/hierarchyConfig';
import { generateColorVariants, createHatchPattern } from '@/lib/maplibre/colorFamilies';
import { isGrandCru } from '@/lib/maplibre/grandCruList';
import { registerTerrainAnalysisProtocol } from '@/lib/maplibre/terrainAnalysis';
...
async function loadCountryGeoJSON(country) {
  ...
  try {
    const res = await fetch(`/data/${country}.geojson`);
...
const MapView = forwardRef(function MapView(
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        paint: {
          ...
          'line-width': [
            ...
            5, ['case', ['boolean', ['feature-state', 'hover'], false], 3, 1.2],
            12, ['case', ['boolean', ['feature-state', 'hover'], false], 3, 1.8],
            15, ['case', ['boolean', ['feature-state', 'hover'], false], 3, 2.5],
          ],
          'line-opacity': 1.0,
        },
      });
      ...
      for (const wineryLayer of ['napa-wineries-marker', 'osm-wineries-marker']) {
