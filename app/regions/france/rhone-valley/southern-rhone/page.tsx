...
const SOUTHERN_RHONE_APPELLATIONS = [
...
export default async function SouthernRhonePage() {
  ...
  const filePath = path.join(guidesDir, 'southern-rhone-guide.md');
        ...
        <aside className="w-64 flex-shrink-0 hidden lg:block">
          <div className="sticky top-8 max-h-[calc(100vh-4rem)] overflow-y-auto pr-2">
            ...
            <nav className="space-y-1">
              {SOUTHERN_RHONE_APPELLATIONS.map((appellation) => (
                <Link
                  key={appellation.slug}
                  href={`/regions/france/southern-rhone/${appellation.slug}`}
