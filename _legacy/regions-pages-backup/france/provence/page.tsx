...
const PROVENCE_APPELLATIONS = [
  { name: 'Côtes de Provence', slug: 'cotes-de-provence' },
  { name: 'Bandol', slug: 'bandol' },
  { name: 'Cassis', slug: 'cassis' },
  { name: "Coteaux d'Aix-en-Provence", slug: 'coteaux-daix-en-provence' },
  { name: 'Coteaux Varois en Provence', slug: 'coteaux-varois-en-provence' },
  { name: 'Les Baux-de-Provence', slug: 'les-baux-de-provence' },
  { name: 'Palette', slug: 'palette' },
  { name: 'Bellet', slug: 'bellet' },
  { name: 'Pierrevert', slug: 'pierrevert' }
] as const;

export default async function ProvencePage() {
  const guidesDir = path.join(process.cwd(), 'guides');
  const filePath = path.join(guidesDir, 'provence-guide.md');
      ...
      <div className="flex gap-8">
        <aside className="w-64 flex-shrink-0 hidden lg:block">
          <div className="sticky top-8 max-h-[calc(100vh-4rem)] overflow-y-auto pr-2">
            ...
            <nav className="space-y-1">
              {PROVENCE_APPELLATIONS.map((appellation) => (
                <Link
                  key={appellation.slug}
                  href={`/regions/france/provence/${appellation.slug}`}
        ...
        <main className="flex-1 max-w-5xl">
          ...
          <header className="mb-8 pb-6 border-b border-gray-200">
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Provence</h1>
